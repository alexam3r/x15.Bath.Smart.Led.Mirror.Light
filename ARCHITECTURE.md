# ARCHITECTURE — Smart Mirror RGBW, прошивка v1.5.0
...
| удержание ≥ 500 мс | `HoldStart` | `OFF`/`SLIDE_OFF` (v1.5.0) → переключить `nightMode` и **заблокировать диммирование до конца удержания**, свет не включается. Включение ночного режима подтверждается одной вспышкой тёмного кольца, снятие — двумя (см. ниже); при снятии ещё и `cooldown` 15 с — PIR не зажжёт свет ни во время удержания, ни сразу после (Ruling R14). В `SLIDE_ON`/`ON` — ничего, яркость меняют `HoldTick`. До v1.5.0 удержание из `OFF` включало свет (§11.1 п. 6), а ночной режим снимало в любом состоянии |
...
|---|---|---|
| `automation` (по умолчанию `true`) | `motion/set`; клик, включающий свет (v1.5.0) | Главный выключатель автоматики. `false` → нет автовключения, автовыключения и автоэффектов. **Постоянный** до явного `motion/set ON`. |
...
### 9.1 Размер прошивки: v27 → v1.5.0

| env | Flash (v27 baseline) | Flash (v1.5.0) | RAM (v27 baseline) | RAM (v1.5.0) |
|---|---|---|---|---|
| `esp32-s3-zero` (прод) | 719081 B | 734629 B (+15548 B, +2.16%) | 44848 B | 47376 B (+2528 B, +5.64%) |
| `esp32-s3-zero-debug` | 720725 B | 736325 B (+15600 B, +2.16%) | 44848 B | 47376 B (+2528 B, +5.64%) |

Прирост v1.4.2 → v1.5.0: Flash +604 Б, RAM +24 Б (ночной режим с кнопки и вспышки подтверждения).
...
**Тесты (v1.5.0):** 257 на ПК (в v1.4.2 — 245, в v1.4.1 — 242, в v1.3.2 — 241, в v1.2.1 — 233) (`pio test -e native`, ~20 с), из них набор `test_soak` (~11 с) — симуляция
