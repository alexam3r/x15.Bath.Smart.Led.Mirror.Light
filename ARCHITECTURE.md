...
1. **Watchdog** (`NetWatchdog`, v1.2.1) — на каждом проходе меряет время без WiFi; > 5 мин подряд →
   `ESP.restart()`, **но только когда зеркало не горит** (`latest.on == false`): горящее зеркало перезагрузка
   ждёт до выключения. Отключение **только MQTT** при живом WiFi перезагрузку не вызывает вовсе — мёртвый
   брокер (HA обновляется полчаса) перезагрузка не лечит, задача просто продолжает переподключаться, а зеркало
   работает локально. В v1.2.0 и v27 правило было «WiFi или MQTT > 5 мин суммарно → сразу», и при долгом
   простое брокера зеркало гасло каждые ~5,5 мин прямо во время использования. Простой считается между
   проходами, поэтому «моргающий» WiFi засчитывается только за реальное время без связи. Почтовый ящик
   снимков теперь читается в начале каждого прохода, в том числе без сети, — иначе `latest.on` устаревал бы.
