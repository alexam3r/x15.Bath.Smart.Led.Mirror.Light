# ARCHITECTURE — Smart Mirror RGBW, прошивка v1.4.0
...
| `flame` | временный (v1.4.0, заменил `breathe`) | «пламя»: «дышат» отдельные участки — блоки шириной 10–15 светодиодов (`FLAME_WIDTH_MIN..MAX`) вместе с мягким краем `FLAME_EDGE` = 3 светодиода с каждой стороны (¼, ½, ¾ глубины, равные для глаза шаги); ровное ядро блока по косинусу гаснет до `FLAME_FLOOR` = 153 (60 % **для глаза**, CIE 1931 — глубина прежнего `breathe`) и возвращается за 3–5 с. Каждый блок на своих часах: новый — с вероятностью 15 % за шаг в случайном месте, не ближе 2 светодиодов к другим (большинство попыток попадает на занятое место), не больше 8 разом, в среднем видно 4–5. 667 шагов × 30 мс ≈ 20 с; в последние 5 с новых нет — эффект затихает сам. `breathe` (v1.2.0–v1.3.2) гасил всё кольцо разом: 3 цикла по 4 с до той же глубины, ≈ 12 с |
...
### 9.1 Размер прошивки: v27 → v1.4.0

| env | Flash (v27 baseline) | Flash (v1.4.0) | RAM (v27 baseline) | RAM (v1.4.0) |
|---|---|---|---|---|
| `esp32-s3-zero` (прод) | 719081 B | 733665 B (+14584 B, +2.03%) | 44848 B | 47336 B (+2488 B, +5.55%) |
| `esp32-s3-zero-debug` | 720725 B | 735213 B (+14488 B, +2.01%) | 44848 B | 47336 B (+2488 B, +5.55%) |

Прирост v1.3.0 → v1.4.0: Flash +596 Б (`flame` вместо `breathe`), RAM +48 Б (пул из 8 блоков `flame`).
...
**Тесты (v1.4.0):** 242 на ПК (в v1.3.2 — 241, в v1.2.1 — 233) (`pio test -e native`, ~20 с), из них набор `test_soak` (~11 с) — симуляция
