# ARCHITECTURE — Smart Mirror RGBW, прошивка v1.5.1
...
>
> v1.2.0 (22.09.2026; на железе — в составе v1.3.1): плавные переходы цвета и яркости (§4.2), затухание за
...
>
> v1.2.1 (23.09.2026; на железе — в составе v1.3.1): ревизия надёжности для круглосуточной работы — watchdog
...
>
> v1.3.0 (23.09.2026; на железе — в составе v1.3.1): по отзыву владельца — все новые градиенты считаются в
...
>
> v1.3.1 (23.09.2026, **проверена на железе**, тег `v1.3.1`): угольки `embers` глубже — до 10–30 % для глаза — и с
> градиентным краем по 2 светодиода. Прошита вместе со всем из v1.2.0–v1.3.0; владелец: «вроде всё гуд, пока
> оставляем так».
>
> v1.3.2 (23.09.2026): мягкий край анимации включения/выключения — в шкале восприятия CIE 1931 и в полтора
> раза длиннее: 15 светодиодов вместо 10 (`SLIDE_EDGE` = 16 вместо 11), анимация ≈ 3,4 с вместо ≈ 3,2 с
> (§4.1, §6.1).
>
> v1.4.0 (24.09.2026): эффект `breathe` заменён на `flame` — вместо всего кольца разом «дышат» отдельные участки
> по 10–15 светодиодов с мягкими краями, каждый на своих часах, до той же глубины 60 % для глаза, ≈ 20 с
> (§4.2). В `effect_list` HA — `flame` вместо `breathe`.
>
> v1.4.1 (24.09.2026, **проверена на железе**): `flame` был почти незаметен — блоки шире и глубже: ровное ядро
> 10–12 светодиодов и градиент по 3–4 с каждой стороны пропорционально ядру (16–20 всего), ядро гаснет до 30 %
> для глаза вместо 60 % (§4.2). Владелец: «теперь лучше, flame оставляем так».
>
> v1.4.2 (24.09.2026): радужная змейка `rainbow` при каждом запуске перемешивает шесть опорных цветов в случайном
> порядке; между ними цвет идёт кратчайшим путём по цветовому кругу. Остальное — как в v27 (§4.2).
>
> v1.5.0 (25.09.2026): ночной режим с кнопки — удержание при выключенном свете включает и выключает его, тёмное
> кольцо подтверждает одной или двумя вспышками; включение света кликом заодно включает автоматику, чтобы
> зеркало, включённое рукой, не горело бесконечно (§4.3, §4.4).
>
> v1.5.1 (25.09.2026): ночной режим с кнопки переключает и автоматику — выключает её вместе с ним и включает
> при снятии, чтобы выключатель «автодетекция движения» в HA показывал, зажигает ли датчик свет (§4.3).
>
...
- Слайд рисует базовый цвет с мягким краем `SLIDE_EDGE = 16` — 15 светодиодов между тёмным и полным
  цветом: яркость для глаза растёт по краю линейно и переводится в ШИМ через `cie8()` (§6.1).
  `maxRadius = 168/2 + 16 + 2 = 102`, шаг 33 мс → 102 × 33 ≈ 3,4 с. Так с v1.3.2; в v1.0.1 край был
  `SLIDE_EDGE = 11` (10 светодиодов) и линейный в ШИМ — его тёмная половина почти не видна глазу, 97 × 33 ≈
  3,2 с; в v27 — `SLIDE_EDGE = 9`, 95 шагов × 28 мс ≈ 2,7 с. Центр слайда при включении из `OFF` — случайный из `CENTERS = {9, 51, 93, 134}`.
- Выключение до первого шага slide-in (радиус ещё 0, т.е. в пределах ≈ 33 мс после включения) продолжает
  ...
  всё кольцо вспыхивало и гасло полным slide-out).
...
| `rainbow` | временный (порядок цветов — v1.4.2) | радужная змейка той же геометрии, смешивание с базой по альфе. Шесть опорных цветов — красный, жёлтый, зелёный, голубой, синий, фиолетовый — стоят через 10 светодиодов от головы; с v1.4.2 при каждом запуске они перемешиваются в случайном порядке (Фишер — Йетс, все 720 порядков равновероятны), между соседними опорными цвет идёт кратчайшим путём по цветовому кругу (для противоположных — по часовой), хвост возвращается к цвету головы. В v27 порядок всегда был красный → жёлтый → … → фиолетовый; `RandomFn`, возвращающий 0, сохраняет его |
...
| `flame` | временный (v1.4.0, заменил `breathe`; шире и глубже в v1.4.1) | «пламя»: «дышат» отдельные участки — блок из ровного ядра `FLAME_CORE_MIN..MAX` = 10–12 светодиодов и мягкого края с каждой стороны, пропорционального ядру (`flameEdge(core)` = треть ядра с округлением: 10 → 3, 11 → 4, 12 → 4; равные для глаза шаги, у края 3 — ¼, ½, ¾ глубины), всего 16–20 светодиодов; ядро по косинусу гаснет до `FLAME_FLOOR` = 77 (30 % **для глаза**, CIE 1931) и возвращается за 3–5 с. Каждый блок на своих часах: новый — с вероятностью 15 % за шаг в случайном месте, не ближе 2 светодиодов к другим (большинство попыток попадает на занятое место), не больше 8 разом, в среднем видно около 4. В v1.4.0 блоки были 10–15 светодиодов вместе с краем 3 и гасли до 60 % — на зеркале почти незаметно. 667 шагов × 30 мс ≈ 20 с; в последние 5 с новых нет — эффект затихает сам. `breathe` (v1.2.0–v1.3.2) гасил всё кольцо разом: 3 цикла по 4 с до той же глубины, ≈ 12 с |
...
|---|---|---|
| 1 клик | `Click(1)` | `OFF`/`SLIDE_OFF` → `automation = true` (v1.5.0), `powerOn()`; иначе `powerOff(manual)` |
| 2 клика | `Click(2)` | `OFF`/`SLIDE_OFF` → `base = Makeup`, `automation = true` (v1.5.0), `powerOn()`; иначе переключить `Solid ↔ Makeup` |
| 3 клика | `Click(3)` | `startEffect(random)` (в `OFF` игнорируется) |
| ≥ 4 клика | `Click(n)` | игнор |
| удержание ≥ 500 мс | `HoldStart` | `OFF`/`SLIDE_OFF` (v1.5.0) → переключить `nightMode`, а `automation` — вслед за ним: `false` при включении ночного режима, `true` при снятии (v1.5.1, чтобы HA видел, работает ли датчик); **заблокировать диммирование до конца удержания**, свет не включается. Включение ночного режима подтверждается одной вспышкой тёмного кольца, снятие — двумя (см. ниже); при снятии ещё и `cooldown` 15 с — PIR не зажжёт свет ни во время удержания, ни сразу после (Ruling R14); запускается после `automation = true`, чей переход `false → true` паузу сбрасывает. В `SLIDE_ON`/`ON` — ничего, яркость меняют `HoldTick`. До v1.5.0 удержание из `OFF` включало свет (§11.1 п. 6), а ночной режим снимало в любом состоянии |
...
**Подтверждение ночного режима (v1.5.0).** Флаг переключается сразу, а тёмное кольцо показывает результат
тёплым цветом по умолчанию: включение — одна вспышка `SIGNAL_ON_FLASH_MS` = 1 с, снятие — две по
`SIGNAL_OFF_FLASH_MS` = 400 мс через 200 мс. Каждая вспышка — косинус до `SIGNAL_LEVEL` = 77 (30 % **для глаза**,
CIE 1931), кадр раз в 20 мс (`Blink` — только тайминг, кадр рисует `Mirror::tickConfirmation`). Сигнал
фиксированный: цвет и яркость, пришедшие из HA при выключенном свете, его не меняют. Удержание во время
slide-out откладывает вспышку до `OFF`. Любое включение света обрывает вспышку и отбрасывает отложенную.
**Клик, включающий свет, включает и автоматику (v1.5.0):** иначе зеркало, включённое рукой при выключенной
в HA автодетекции, горело бы до ручного выключения. Включение из HA, Алисы и PIR флаг не трогает.
...
|---|---|---|
| `automation` (по умолчанию `true`) | `motion/set`; клик, включающий свет (v1.5.0); удержание кнопки при выключенном свете — вместе с ночным режимом (v1.5.1) | Главный выключатель автоматики. `false` → нет автовключения, автовыключения и автоэффектов. Держится до `motion/set ON` или кнопки. |
| `nightMode` (по умолчанию `false`) | `motion_disable/set`, удержание кнопки при выключенном свете (вкл/выкл, v1.5.0), любое `powerOn()` | «Глухое» отключение PIR (Node-RED или кнопка, ночь). При включении — если свет горит, гасится. |
...
| `flame` (v1.4.0; раньше `breathe`) | косинус блока и ступени его края — яркость для глаза |
...
| мягкий край slide (§4.1, v1.3.2) | яркость для глаза растёт по краю линейно, `cie8()` по ней |

**Не трогаются** (решение владельца): змейки `dark`/`rainbow` и волна `wave` из v27 — их вид одобрен вживую, а
тесты сверяют их кадры с v27 байт в байт (`rainbow` — при исходном порядке цветов, v1.4.2). Мягкий край slide-анимации был в этом списке до v1.3.2; владелец
попросил перевести его в шкалу восприятия и удлинить в полтора раза. Ползунок яркости HA тоже не переводится:
...
// enum class EffectId : uint8_t { None = 0, Dark, Rainbow, Wave, Flame, Embers, Comet };

static Comet        s_comet;
static DarkSnake    s_dark;
static Embers       s_embers;
static Flame        s_flame;
...
static const EffectEntry kEffects[] = {
    ...
    {EffectId::Flame,   "flame",   &s_flame},     // v1.4.0, заменил breathe (v1.2.0)
...
│       ├── Blink.{h,cpp}           # тайминг вспышек подтверждения ночного режима (v1.5.0)
...
enum class EffectId   : uint8_t { None = 0, Dark, Rainbow, Wave, Flame, Embers, Comet };   // временные эффекты (см. kEffects[])
...
### 9.1 Размер прошивки: v27 → v1.5.1

| env | Flash (v27 baseline) | Flash (v1.5.1) | RAM (v27 baseline) | RAM (v1.5.1) |
|---|---|---|---|---|
| `esp32-s3-zero` (прод) | 719081 B | 734637 B (+15556 B, +2.16%) | 44848 B | 47376 B (+2528 B, +5.64%) |
| `esp32-s3-zero-debug` | 720725 B | 736337 B (+15612 B, +2.17%) | 44848 B | 47376 B (+2528 B, +5.64%) |

Прирост v1.4.2 → v1.5.1: Flash +612 Б, RAM +24 Б (ночной режим с кнопки и вспышки подтверждения; в v1.5.1 —
ещё +8 Б на автоматику вслед за ночным режимом).

Прирост v1.4.1 → v1.4.2: Flash +264 Б, RAM +16 Б (порядок цветов радуги).

Прирост v1.3.0 → v1.4.1: Flash +692 Б (`flame` вместо `breathe`; в v1.4.1 — ещё +96 Б на край, пропорциональный
ядру), RAM +48 Б (пул из 8 блоков `flame`).
...
**Тесты (v1.5.1):** 257 на ПК (в v1.4.2 — 245, в v1.4.1 — 242, в v1.3.2 — 241, в v1.2.1 — 233) (`pio test -e native`, ~20 с), из них набор `test_soak` (~11 с) — симуляция
...
1. **`effect` в JSON Light** (вариант A): `solid, makeup, random, dark, rainbow, wave, flame, embers, comet` (`flame` — с v1.4.0, вместо `breathe`). Топики `makeup/set` и `effect/set` сохранены.
...
6. **Удержание кнопки из `OFF`** включает зеркало со slide-анимацией, диммирование начинается после выхода в `ON`
   (в v27 — мгновенная заливка без анимации). С v1.5.0 — не так: удержание из `OFF` переключает ночной режим (п. 32).
...
18. **v1.2.0: четыре новых временных эффекта** — `breathe`, `embers`, `candle`, `comet` (`candle` убран в v1.3.0, `breathe` заменён на `flame` в v1.4.0); все в `effect_list`
...
27. **v1.3.0: `candle` убран, `embers` переделан** в редкие угольки с мягкими соседями, края глитча — 4–6
    светодиодов (§4.2, §4.6).
28. **v1.3.2: мягкий край анимации включения/выключения — в шкале восприятия и в полтора раза длиннее**:
    15 светодиодов вместо 10, ровные для глаза ступени, ≈3,4 с вместо ≈3,2 с (§4.1, §6.1).
29. **v1.4.0: `breathe` заменён на `flame`** — «дышат» отдельные участки по 10–15 светодиодов с мягкими краями,
    каждый на своих часах, до 60 % для глаза, ≈ 20 с вместо ≈ 12 с; имя `breathe` больше не принимается (§4.2).
30. **v1.4.1: `flame` шире и глубже** — ядро 10–12 светодиодов плюс градиент 3–4 с каждой стороны
    пропорционально ядру (16–20 всего вместо 10–15), до 30 % для глаза вместо 60 % (§4.2).
31. **v1.4.2: радуга в случайном порядке** — шесть опорных цветов `rainbow` при каждом запуске перемешиваются,
    переходы между ними — кратчайшим путём по цветовому кругу; геометрия, скорость и появление прежние (§4.2).
32. **v1.5.0: ночной режим с кнопки** — удержание при выключенном свете (или во время slide-out) включает и
    выключает ночной режим, свет не зажигается и яркость не меняется; тёмное кольцо подтверждает: одна вспышка —
    включён, две — снят (§4.3).
33. **v1.5.0: клик, включающий свет, включает и автоматику** — зеркало, включённое рукой при выключенной
    автодетекции, больше не горит до ручного выключения; включение из HA/Алисы флаг не трогает (§4.3, §4.4).
34. **v1.5.1: ночной режим с кнопки переключает и автоматику** — удержание, включающее ночной режим,
    выключает «автодетекцию движения», снимающее — включает; в HA видно, зажигает ли датчик свет (§4.3).
    Команда `motion_disable/set` из Node-RED автоматику по-прежнему не трогает.

Без изменений: пины, количество LED, кольцевое отображение, скорости/размеры старых эффектов (кроме slide с
v1.0.1 и v1.3.2), дефолтный цвет, сброс настроек при выключении, таймеры автоэффекта и PIR (4–5 мин / 15 с / 2 с),
...
- **Тесты:** `slide_on_takes_97_steps` (радиусы 0..96); `slide_off_takes_98_steps` (радиусы 97..0); `slide_reverse_keeps_radius`
  (с v1.3.2 — `slide_on_takes_max_radius_steps`, `slide_off_takes_max_radius_plus_one_steps`, 102/103 шага);
  ...
  (кадры slide с v1.3.2 сверяются с формулой края в шкале восприятия — `slide_frames_follow_the_perceptual_edge`).
