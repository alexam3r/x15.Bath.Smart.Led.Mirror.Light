# ARCHITECTURE — Smart Mirror RGBW, прошивка v1.1.0
...
| `random` | действие | случайный из `dark/rainbow/wave`; в `state` отражается реально запущенный |
...
Короткая имитация помехи: случайный участок из 3–6 соседних светодиодов (`GLITCH_LEN_MIN..MAX`) в любом
...
### 9.1 Размер прошивки: v27 → v1.1.0

| env | Flash (v27 baseline) | Flash (v1.1.0) | RAM (v27 baseline) | RAM (v1.1.0) |
|---|---|---|---|---|
| `esp32-s3-zero` (прод) | 719081 B | 721161 B (+2080 B, +0.29%) | 44848 B | 46904 B (+2056 B, +4.58%) |
| `esp32-s3-zero-debug` | 720725 B | 722297 B (+1572 B, +0.22%) | 44848 B | 46904 B (+2056 B, +4.58%) |
...
1. **`effect` в JSON Light** (вариант A): `solid, makeup, random, dark, rainbow, wave`. Топики `makeup/set` и `effect/set` сохранены.
