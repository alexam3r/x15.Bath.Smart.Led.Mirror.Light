# ARCHITECTURE — Smart Mirror RGBW, прошивка v1.2.0
...
| `comet` | временный (v1.2.0) | «метеор»: белая голова с хвостом 40 LED (квадратичное затухание) из случайной точки в случайную сторону, один оборот + хвост; 209 шагов × 30 мс ≈ 6,3 с |
...
```json
{"uptime_s":3723,"rssi":-61,"reset_reason":"POWERON","free_heap":231000,"min_free_heap":198000,"max_alloc_heap":110592,"fw":"1.2.0"}
```
...
В HA — пять диагностических сенсоров (`ha_mirror.yaml`); буфер `DIAG_JSON_CAP` = 192 Б, при нехватке
...
### 9.1 Размер прошивки: v27 → v1.2.0

| env | Flash (v27 baseline) | Flash (v1.2.0) | RAM (v27 baseline) | RAM (v1.2.0) |
|---|---|---|---|---|
| `esp32-s3-zero` (прод) | 719081 B | 729865 B (+10784 B, +1.50%) | 44848 B | 47432 B (+2584 B, +5.76%) |
| `esp32-s3-zero-debug` | 720725 B | 731109 B (+10384 B, +1.44%) | 44848 B | 47432 B (+2584 B, +5.76%) |
...
1. **`effect` в JSON Light** (вариант A): `solid, makeup, random, dark, rainbow, wave, breathe, embers, candle, comet`. Топики `makeup/set` и `effect/set` сохранены.
...
18. **v1.2.0: четыре новых временных эффекта** — `breathe`, `embers`, `candle`, `comet`; все в `effect_list`
