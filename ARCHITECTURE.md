...
4. **Снимок** — `xQueueReceive(snapQueue, &snap, 0)`; при новом снимке — публикация, но не чаще 1 раза в 250 мс
   (mailbox всегда хранит последнее состояние, поэтому финальное значение гарантированно уйдёт).
...
5. **Status LED**: жёлтый — подключение WiFi, синий — подключение MQTT, зелёная вспышка 1 с — MQTT подключён,
   короткое тусклое зелёное мигание — публикация телеметрии (как v27).
...
| `powerOff(manual)` | — | → `SLIDE_OFF`, радиус текущий | → `SLIDE_OFF`, радиус max; эффект отменяется | — |
...
- Размер ≈ 260 байт → буфер сериализации 384 байта, MQTT-буфер 512 байт.
