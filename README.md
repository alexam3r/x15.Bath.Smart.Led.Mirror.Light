...
Прошивка (v1.0.0, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **Serial-логи** — PIR rising-edge (не спамит на каждом тике цикла), mode transitions, авто-выключение, эффекты. Включаются флагом сборки `DEBUG_LOG_ENABLED` — окружение `esp32-s3-zero-debug` (1) или `esp32-s3-zero` (0, продакшен), см. `platformio.ini`.
...
|---|---|---|
| `state` | да | JSON-статус: `state`, `brightness`, `color`, `color_mode`, `effect`, `automation`, `night_mode`, `fw`, + устаревшие совместимые поля (`brightness_pct`, `moveDetection`, `makeup`) |
...
`firmware/lib/MirrorCore`: без Arduino/FreeRTOS, тестируется на ПК (`pio test -e native`, 117 тестов).
...
| Удержание ≥ 0.5 с | Если включён ночной режим (`motion_disable/set ON`) — снимает его и блокирует диммирование до конца этого удержания (свет сам не загорается). Если зеркало было выключено — включает его slide-анимацией, диммирование начинается после её завершения. Иначе — плавное диммирование (пила 5↔255, шаг 5 каждые 30 мс) |
...
6. Во время эффектов при мигании статус-светодиода (публикация телеметрии раз в 10 с) нет сбоев кадра и
   перезагрузок (известный риск R12 библиотеки NeoPixel).
...
├── ARCHITECTURE.md                    # спецификация модульной архитектуры v1.0.0
...
- **v1.0.0** (текущая, ветка `refactor/v1`) — порт прошивки v27 с монолитного `main.cpp` на модульную
  архитектуру (`firmware/lib/MirrorCore` — чистая тестируемая логика, `firmware/src` — железо и сеть, две
  очереди FreeRTOS вместо мьютекса и `volatile`-глобалов, 117 unit-тестов на ПК). Нумерация версий начата
  ...
  единственная точка переходов состояния — конечный автомат `Mirror` (полный список — ARCHITECTURE.md §11.2).
