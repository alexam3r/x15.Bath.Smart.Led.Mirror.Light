...
Прошивка (v1.5.0, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
25. Ночной режим с кнопки (v1.5.0): при выключенном свете подержать кнопку — одна мягкая вспышка, свет не
    включается, датчик движения больше не зажигает зеркало (проверить, пройдясь перед ним через 20 с).
    Подержать ещё раз — две вспышки, через 15 с датчик снова работает. Подержать сразу после выключения
...
- **v1.5.0** (текущая) — ночной режим с кнопки: при выключенном свете удержание включает и выключает его
