...
Прошивка (v1.4.0, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **6 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  (`rainbow`), волна с затемнением (`wave`), «пламя» (`flame`, v1.4.0) — участки по 10–15 светодиодов с
  мягкими краями, каждый в своё время, плавно гаснут до 60 % (для глаза) за 3–5 секунд и возвращаются, ~20 секунд,
...
23. «Пламя» (v1.4.0): сначала обновить `ha_mirror.yaml` в HA (в списке эффектов `flame` вместо `breathe`) и
    перезагрузить MQTT. `flame` — около 20 с в разных местах кольца по очереди плавно гаснут и возвращаются
    участки по 10–15 светодиодов с мягкими краями, 4–5 одновременно, каждый в своём ритме; всё кольцо разом не
    гаснет. Проверить и в solid, и в Макияже.
...
- **v1.4.0** (текущая) — эффект «дыхание» (`breathe`) заменён на «пламя» (`flame`): вместо всего кольца разом
