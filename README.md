...
- **6 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  ...
  возвращается три раза, «угли» (`embers`, v1.2.0) — каждый светодиод тлеет на своём уровне 40–100 %, ~20 секунд, и «свеча»
  (`candle`, v1.2.0) — всё кольцо дрожит, как пламя, изредка проваливаясь до 80 %.
- **JSON Light `effect`** — одно поле в команде `set` принимает `solid, makeup, random, dark, rainbow, wave, breathe, embers, candle`:
...
|---|---|---|
| `set` | JSON: `state`, `brightness`, `color`, `effect` | Полное управление (HA JSON Light). `effect` принимает `solid, makeup, random, dark, rainbow, wave, breathe, embers, candle` |
...
|---|---|---|
| light «Зеркало в душевой» — JSON Light: `brightness_scale: 255`, `supported_color_modes: [rgb]`, `effect_list: solid, makeup, random, dark, rainbow, wave, breathe, embers, candle` | `mirror_bath_flat8` | `set` / `state` |
...
- **v1.0.0** (проверена на железе 22.09.2026 — работает так же, как v27) — порт прошивки
  ...
  Намеренные изменения поведения: `effect` в JSON Light (`solid, makeup, random, dark, rainbow, wave, breathe, embers, candle`);
