...
Прошивка (v1.3.0, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
19. «Угли» (v1.3.0): отдельные участки по 3 светодиода в разное время мягко гаснут заметно глубже, а не всё
    кольцо рябит; не похоже на выпавшие пиксели.
...
- **v1.3.0** (текущая; на железе ещё не проверена) — по отзыву с зеркала: все новые градиенты считаются в
