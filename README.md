...
- **7 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  (`rainbow`), волна с затемнением (`wave`), «дыхание» (`breathe`, v1.2.0) — кольцо плавно гаснет до 60 % и
  возвращается три раза, «угли» (`embers`, v1.2.0) — каждый светодиод тлеет на своём уровне 40–100 %, ~20 секунд, «свеча»
  (`candle`, v1.2.0) — всё кольцо дрожит, как пламя, изредка проваливаясь до 80 %, и «метеор» (`comet`,
  v1.2.0) — белая голова с гаснущим хвостом облетает кольцо за 6 секунд.
- **JSON Light `effect`** — одно поле в команде `set` принимает `solid, makeup, random, dark, rainbow, wave, breathe, embers, candle, comet`:
...
|---|---|---|
| `set` | JSON: `state`, `brightness`, `color`, `effect` | Полное управление (HA JSON Light). `effect` принимает `solid, makeup, random, dark, rainbow, wave, breathe, embers, candle, comet` |
...
|---|---|---|
| light «Зеркало в душевой» — JSON Light: `brightness_scale: 255`, `supported_color_modes: [rgb]`, `effect_list: solid, makeup, random, dark, rainbow, wave, breathe, embers, candle, comet` | `mirror_bath_flat8` | `set` / `state` |
...
11. Новые эффекты (v1.2.0): `breathe` — дыхание кольца, `embers` — тление отдельных светодиодов,
    `candle` — дрожь пламени, `comet` — белая голова с хвостом делает круг. Проверить и в solid, и в Макияже.
...
17. Команды (v1.2.1): во время предупреждающего затухания сказать Алисе «яркость зеркала 80 %» — затухание
    снимается, таймер начинается заново; выбор `solid` в списке эффектов HA во время `candle` сразу его
    останавливает.
...
- **v1.0.0** (проверена на железе 22.09.2026 — работает так же, как v27) — порт прошивки
  ...
  Намеренные изменения поведения: `effect` в JSON Light (`solid, makeup, random, dark, rainbow, wave, breathe, embers, candle, comet`);
