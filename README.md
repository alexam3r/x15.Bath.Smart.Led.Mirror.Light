...
Прошивка (v1.4.1, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **6 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  (`rainbow`), волна с затемнением (`wave`), «пламя» (`flame`, v1.4.0–1.4.1) — участки по 10–12 светодиодов
...
- **v1.4.1** (текущая) — «пламя» было почти незаметно: блоки шире — ровное ядро 10–12 светодиодов плюс
