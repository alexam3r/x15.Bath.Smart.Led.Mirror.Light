...
Прошивка (v1.3.2, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **6 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  (`rainbow`), волна с затемнением (`wave`), «дыхание» (`breathe`, v1.2.0) — кольцо плавно гаснет до 60 % (для глаза, с v1.3.0) и
  возвращается три раза, «угли» (`embers`, v1.2.0, переделан в v1.3.0–1.3.1) — отдельные пятна
...
21. Шкала CIE (v1.3.0): дыхание (`breathe`), хвост метеора и затухание перед выключением — плавные на глаз
...
- **v1.3.2** (текущая) — мягкий край анимации включения/выключения в шкале яркости для глаза CIE 1931 и в
