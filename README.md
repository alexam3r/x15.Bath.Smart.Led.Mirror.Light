...
Прошивка (v1.1.0, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **Глитч «сбой неона»** (v1.1.0) — примерно раз в минуту (случайно 45–90 с) случайный участок из 3–6 соседних
  светодиодов на 300–600 мс мерцает, как барахлящая неоновая трубка: гаснет, тлеет, вспыхивает. Только в solid,
...
`firmware/lib/MirrorCore`: без Arduino/FreeRTOS, тестируется на ПК (`pio test -e native`, 146 тестов).
...
6. Во время эффектов и диммирования при мигании статус-светодиода (мигает только на heartbeat `state` раз
   ...
   библиотеки NeoPixel).
7. Глитч (v1.1.0): в solid примерно раз в минуту на полсекунды мерцают 3–6 соседних светодиодов; в Макияже,
...
- **v1.1.0** (текущая) — глитч «сбой неона»: примерно раз в минуту (45–90 с) 3–6 соседних светодиодов на
