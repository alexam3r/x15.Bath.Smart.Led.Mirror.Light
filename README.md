...
- **«Глухое» отключение PIR через MQTT** — отдельная команда `motion_disable/set` для Node-RED: «на ночь отключить реакцию на PIR». Не уходит в HA discovery, в `motion/set ON`/`OFF` обычной автоматики **не пересекается** — это независимый флаг. Если в момент команды зеркало горит, оно само гаснет. Снимается командой `OFF` или удержанием кнопки (≥0.5 с). Любое включение (кнопка / MQTT / makeup-on) автоматически снимает disable.
...
#### Сценарии `motion_disable`

| До команды | Команда | После команды | Логика |
|---|---|---|---|
| Свет горит, PIR активен | `motion_disable = ON` | Свет погашен, PIR выкл | Зеркало выключается через `triggerPowerOff()` (slide-OUT) + ставится «глухой» флаг |
| Свет выкл, PIR активен | `motion_disable = ON` | Свет выкл, PIR выкл | Флаг ставится, никакого включения |
| Свет выкл, PIR выкл | `motion_disable = OFF` | Свет выкл, PIR активен | Флаг снимается; зеркало остаётся выключенным, но PIR уже реагирует |
| Только что ручной OFF, идёт 15-сек кулдаун | `motion_disable = OFF` | Свет выкл, PIR активен | Кулдаун отменяется, PIR реактивируется **немедленно** |
| Любое включение (кнопка / MQTT) | (авто) | Свет горит, PIR активен | `triggerPowerOn()` снимает `pirDisabledByUser` автоматически — пользователь включил свет, значит PIR нужен |
