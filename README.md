...
Прошивка (v1.0.1, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
`firmware/lib/MirrorCore`: без Arduino/FreeRTOS, тестируется на ПК (`pio test -e native`, 130 тестов).
...
> v1.0.1 (более медленная анимация включения/выключения) на железе ещё не проверялась.
...
- **v1.0.1** (текущая) — анимация включения/выключения на ~20% медленнее и мягче: шаг 33 мс вместо 28
