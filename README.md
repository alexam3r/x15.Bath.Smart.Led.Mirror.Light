...
`firmware/lib/MirrorCore`: без Arduino/FreeRTOS, тестируется на ПК (`pio test -e native`, 124 теста).
...
- **v1.0.0** (текущая, ветка `refactor/v1`) — порт прошивки v27 с монолитного `main.cpp` на модульную
  ...
  очереди FreeRTOS вместо мьютекса и `volatile`-глобалов, 124 unit-теста на ПК). Нумерация версий начата
