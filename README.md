...
- **Network watchdog** — если Wi-Fi недоступен **более 5 минут**, ESP перезагружается, но только когда свет выключен: горящее зеркало ждёт, пока его выключат. Пропажа одного MQTT-брокера (HA обновляется) перезагрузку не вызывает — зеркало продолжает переподключаться и работает локально (v1.2.1). Локальная работа зеркала (PIR, кнопка, эффекты) при отсутствии сети не страдает.
