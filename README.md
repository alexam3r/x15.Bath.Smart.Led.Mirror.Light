...
Прошивка (v1.2.1, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **Диагностика в Home Assistant** (v1.2.0) — отдельный топик `diag` раз в минуту: аптайм, уровень WiFi,
  ...
  (v1.2.1), версия прошивки. Пять диагностических сенсоров; в основной статус светильника не попадает, чтобы не дёргать его каждую секунду.
...
- **v1.2.1** (текущая; на железе ещё не проверена) — ревизия надёжности для круглосуточной работы. Медленный
