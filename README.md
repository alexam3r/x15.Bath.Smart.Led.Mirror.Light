...
Прошивка (v1.5.1, модульная архитектура — см. [ARCHITECTURE.md](ARCHITECTURE.md)) оптимизирована для
...
- **6 временных эффектов** (запускаются поверх текущего режима): тёмная змейка (`dark`), радужная змейка
  (`rainbow`; с v1.4.2 шесть её цветов при каждом запуске идут в случайном порядке), волна с затемнением (`wave`), «пламя» (`flame`, v1.4.0–1.4.1) — участки по 10–12 светодиодов
  с мягкими краями по 3–4, каждый в своё время, плавно гаснут до 30 % (для глаза) за 3–5 секунд и возвращаются, ~20 секунд,
  «угли» (`embers`, v1.2.0, переделан в v1.3.0–1.3.1) — отдельные пятна
  по очереди, каждое в своё время, мягко гаснут до 10–30 % с плавным краем по 2 светодиода и возвращаются, ~20 секунд,
  и «метеор» (`comet`, v1.2.0) — белая голова с гаснущим хвостом облетает кольцо за 6 секунд. «Свеча»
  (`candle`) была в v1.2.0 и убрана в v1.3.0: её мерцание было почти незаметно. «Дыхание» (`breathe`,
  v1.2.0–v1.3.2) гасило всё кольцо разом; в v1.4.0 его заменило «пламя».
- **JSON Light `effect`** — одно поле в команде `set` принимает `solid, makeup, random, dark, rainbow, wave, flame, embers, comet`:
  базовые режимы и запуск эффектов не требуют отдельных топиков.
- **Аппаратная кнопка** (триггер Шмитта SN74LVC2G14): 1 клик — вкл/выкл, 2 клика — toggle Макияж, 3 клика — случайный эффект, удержание — плавное диммирование при горящем свете, а при выключенном — ночной режим (v1.5.0).
...
- **Предупреждение за минуту до автовыключения** (v1.2.0) — свет плавно притухает вдвое (для глаза, с v1.3.0) за 2 с: махните рукой,
...
|---|---|---|
| `set` | JSON: `state`, `brightness`, `color`, `effect` | Полное управление (HA JSON Light). `effect` принимает `solid, makeup, random, dark, rainbow, wave, flame, embers, comet` |
...
| `motion_disable/set` | switch-payload | «Глухое» отключение PIR. НЕ в HA, для Node-RED; с кнопки — удержанием при выключенном свете (v1.5.0). Снимается `OFF`, удержанием кнопки (≥0.5 с) или включением света. Повторный `OFF`, когда ночной режим уже снят, ничего не меняет и не отменяет 15-секундную паузу PIR |
...
|---|---|---|
| light «Зеркало в душевой» — JSON Light: `brightness_scale: 255`, `supported_color_modes: [rgb]`, `effect_list: solid, makeup, random, dark, rainbow, wave, flame, embers, comet` | `mirror_bath_flat8` | `set` / `state` |
...
> для Node-RED (ночная автоматика); с v1.5.0 ночной режим ставится и снимается ещё и удержанием кнопки при
> выключенном свете. Обычный переключатель PIR-автоматики в HA/Алисе — через
...
`firmware/lib/MirrorCore`: без Arduino/FreeRTOS, тестируется на ПК (`pio test -e native`, 257 тестов, включая симуляцию долгой работы `test_soak`).
...
| Действие | Эффект |
|---|---|
| 1 короткий клик | Вкл (со slide-анимацией) / Выкл. При включении снимается ночной режим и 15-сек кулдаун, а с v1.5.0 заодно включается автоматика (`motion/set ON`) — зеркало, включённое рукой, погаснет по таймеру, а не будет гореть бесконечно |
| 2 коротких клика | Toggle режима «Макияж» (белый канал W). Если свет выключен — включает сразу в Макияже (и, как клик, включает автоматику) |
| 3 коротких клика | Случайный эффект (работает в обоих режимах) |
| Удержание ≥ 0.5 с | **Свет выключен (или гаснет):** включает/выключает ночной режим (`motion_disable`, v1.5.0) — датчик движения перестаёт/снова начинает зажигать свет. Свет не загорается, яркость не меняется; тёмное кольцо подтверждает: **одна** мягкая вспышка — ночной режим включён, **две** — снят (после снятия PIR ещё 15 с молчит, чтобы не зажечь свет тому, кто держит кнопку). Вместе с ночным режимом переключается и «Зеркало: автодетекция движения» в HA (v1.5.1): выключается при включении ночного режима, включается при снятии — в HA видно, зажигает ли датчик свет. **Свет горит:** плавное диммирование (пила 5↔255, шаг 5 каждые 30 мс). До v1.5.0 удержание при выключенном свете включало его |
...
> вживую — отличная.
> **Результат для v1.3.1:** 23.09.2026 прошита в зеркало (вместе со всем из v1.2.0–v1.3.0); владелец посмотрел
> эффекты и работу зеркала — «вроде всё гуд, пока оставляем так». Тег `v1.3.1`.
> **Результат для v1.4.1:** 24.09.2026 прошита в зеркало; «пламя» — «теперь лучше, flame оставляем так».
...
11. Новые эффекты (v1.2.0): `breathe` — дыхание кольца (с v1.4.0 — `flame`, п. 23), `embers` — тление
    отдельных светодиодов, `comet` — белая голова с хвостом делает круг (`candle` убран в v1.3.0). Проверить и в solid, и в Макияже.
...
19. «Угли» (v1.3.1): отдельные пятна по 5 светодиодов в разное время мягко гаснут почти до темноты (10–30 %)
    с плавным краем, а не всё кольцо рябит; не похоже на выпавшие пиксели.
20. Глитч (v1.3.0): края — плавный градиент на 4–6 светодиодов с каждой стороны.
21. Шкала CIE (v1.3.0): дыхание (`breathe`, с v1.4.0 — `flame`), хвост метеора и затухание перед выключением — плавные на глаз
    и заметно глубже, чем раньше; смена цвета из HA идёт ровно, без «проскакивания» тёмной части.
22. Анимация включения/выключения (v1.3.2): мягкий край заметно длиннее (15 светодиодов вместо 10) и ровный
    на глаз — без резкой ступеньки у полного цвета; вся анимация ≈ 3,4 с; скорость бегущего края прежняя.
23. «Пламя» (v1.4.0, шире и глубже в v1.4.1): сначала обновить `ha_mirror.yaml` в HA (в списке эффектов
    `flame` вместо `breathe`) и перезагрузить MQTT. `flame` — около 20 с в разных местах кольца по очереди
    плавно гаснут почти до трети яркости и возвращаются участки по 10–12 светодиодов с мягкими краями по 3–4,
    около 4 одновременно, каждый в своём ритме; эффект хорошо заметен, всё кольцо разом не гаснет. Проверить
    и в solid, и в Макияже.
24. Радуга (v1.4.2): запустить `rainbow` несколько раз подряд — порядок цветов каждый раз другой, переходы
    между ними плавные и насыщенные, без серых провалов; длина, скорость, появление и исчезание — как раньше.
25. Ночной режим с кнопки (v1.5.0–1.5.1): при выключенном свете подержать кнопку — одна мягкая вспышка, свет
    не включается, датчик движения больше не зажигает зеркало (проверить, пройдясь перед ним через 20 с), в HA
    «Зеркало: автодетекция движения» выключилась. Подержать ещё раз — две вспышки, выключатель в HA включился,
    через 15 с датчик снова работает. Подержать сразу после выключения
    кликом, пока кольцо гаснет, — вспышка после того, как оно погасло. При горящем свете удержание, как и
    раньше, меняет яркость. Выключить в HA «Зеркало: автодетекция движения», включить свет кликом —
    выключатель в HA сам включается, и зеркало гаснет по таймеру.
...
- **v1.5.1** (текущая) — ночной режим с кнопки переключает и «Зеркало: автодетекция движения» в HA: удержание,
  включающее ночной режим (одна вспышка), выключает её, снимающее (две вспышки) — включает. В HA видно,
  зажигает ли датчик свет. 257 тестов.
- **v1.5.0** — ночной режим с кнопки: при выключенном свете удержание включает и выключает его
  (датчик движения перестаёт или снова начинает зажигать свет), тёмное кольцо подтверждает одной мягкой
  вспышкой (включён) или двумя (снят); удержание во время гашения работает так же. Клик, включающий свет,
  заодно включает автоматику — зеркало, включённое рукой, больше не горит до ручного выключения. Раньше
  удержание при выключенном свете включало его. 257 тестов.
- **v1.4.2** (тег `v1.4.2`) — радужная змейка при каждом запуске перемешивает шесть своих цветов (красный, жёлтый,
  зелёный, голубой, синий, фиолетовый) в случайном порядке; между ними цвет перетекает кратчайшим путём по
  цветовому кругу, всегда насыщенный. Остальное в эффекте прежнее. 245 тестов.
- **v1.4.1** (проверена на зеркале 24.09.2026 — «теперь лучше») — «пламя» было почти незаметно: блоки шире — ровное ядро 10–12 светодиодов плюс
  градиент по 3–4 с каждой стороны пропорционально ядру (16–20 всего вместо 10–15), — и глубже: до 30 % для
  глаза вместо 60 %. Около 4 блоков одновременно. 242 теста.
- **v1.4.0** — эффект «дыхание» (`breathe`) заменён на «пламя» (`flame`): вместо всего кольца разом
  «дышат» отдельные участки по 10–15 светодиодов с мягкими краями по 3 светодиода, каждый на своих часах, 4–5
  одновременно, до той же глубины 60 % для глаза за 3–5 с; эффект длится ~20 с. В `ha_mirror.yaml` — `flame`
  вместо `breathe`. 242 теста.
- **v1.3.2** — мягкий край анимации включения/выключения в шкале яркости для глаза CIE 1931 и в
  полтора раза длиннее: 15 светодиодов вместо 10, ровный на глаз градиент без «пустой» тёмной половины;
  анимация ≈ 3,4 с вместо ≈ 3,2 с при той же скорости 33 мс на светодиод. 241 тест.
- **v1.3.1** (тег `v1.3.1`, проверена на зеркале 23.09.2026 — «вроде всё гуд») — «угли» глубже: центр уголька гаснет до 10–30 % для глаза, с градиентным краем по
  2 светодиода с каждой стороны; пятна реже и не ближе 5 светодиодов друг к другу. 239 тестов.
- **v1.3.0** — по отзыву с зеркала: все новые градиенты считаются в
  шкале яркости для глаза CIE 1931 — плавно и заметно, а не «почти незаметно»; «свеча» убрана; «угли»
  переделаны: около каждого восьмого светодиода в своё время мягко гаснет до 30–50 % вместе с соседями;
  края глитча — 4–6 светодиодов; дыхание, затухание перед выключением и хвост метеора стали глубже. 239 тестов.
- **v1.2.2** (на железе — в составе v1.3.1) — сенсор «Зеркало: последний эффект»: имя последнего
...
- **v1.2.1** (на железе — в составе v1.3.1) — ревизия надёжности для круглосуточной работы. Медленный
...
- **v1.2.0** (на железе — в составе v1.3.1) — плавные переходы цвета и яркости (0,5 с); затухание до
