...
void Flame::spawn(const EffectContext& ctx) {
    ...
    const uint8_t width = static_cast<uint8_t>(
        cfg::FLAME_WIDTH_MIN + ctx.random(cfg::FLAME_WIDTH_MAX - cfg::FLAME_WIDTH_MIN + 1));
    ...
    for (const Patch& p : patches_) {
        ...
        for (int k = -static_cast<int>(cfg::FLAME_MIN_GAP); k < p.width + cfg::FLAME_MIN_GAP; ++k) {
...
bool Flame::step(Frame& out, const EffectContext& ctx) {
    ...
    for (Patch& p : patches_) {
        ...
        for (uint8_t j = 0; j < p.width; ++j) {
            const uint8_t fromOutside = j < p.width - 1 - j ? j : static_cast<uint8_t>(p.width - 1 - j);
            const float share = fromOutside < cfg::FLAME_EDGE
                                    ? static_cast<float>(fromOutside + 1) / (cfg::FLAME_EDGE + 1)
                                    : 1.0f;
