 ...
 *  - applyDefaults() срабатывает ПОСЛЕ полного выключения ленты
 *    (в processAnimation при MODE_ANIM_OFF → MODE_OFF), поэтому смена цвета
 *    на дефолт незаметна глазу — слайд-out играет на текущем цвете, и только
 *    после физического гашения настройки сбрасываются для следующего включения.
 *  - PIR_RESUME_DELAY_MS захардкожен на 15 секунд.
    ...
    if (state.currentMode == MODE_ANIM_ON || state.currentMode == MODE_ANIM_OFF) {
        ...
        if (finished) {
            if (stepDir > 0) {
                ...
                // Slide-off завершён. Лента сейчас погашена (буферы очищаются ниже),
                // и только теперь можно без видимого артефакта сбросить цвет/яркость/режим
